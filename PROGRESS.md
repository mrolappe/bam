...
## Rounds 51–55 — 2026-08-08 · Phase 8: manifest/contract versioning → extism host/registry integration → `content_analyzer` → real archive extraction → failure isolation

Full round-by-round log moved to
[docs/progress/phase-8-plugin-host.md](docs/progress/phase-8-plugin-host.md)
to keep this file scannable — it covers P8.1 through P8.5: `PluginManifest`
and `contract_schema` versioning (P8.1), `WasmUnpacker` wired into the
unchanged `UnpackerRegistry` proving I4 for a plugin backend (P8.2), the
`content_analyzer` extension point with per-plugin producer versioning
(P8.3), a WASM-backed unpacker doing real ZIP extraction with a fixed
no-partial-extraction guarantee (P8.4), and `[plugins]` config plus
per-plugin panic/timeout/memory-limit failure isolation (P8.5, Phase 8
exit). 267 tests by the end of the phase (19 added across the five rounds).

**Phase 8 exit reached.** Third parties can extend content analysis and
